package com.revisit.springboot.controller.productsku;
...
public class ProductSkuController {
    ...
    private final static Logger logger = LoggerFactory.getLogger(ProductSkuController.class);
    ...
    public @ResponseBody JSONObject addProductSku(@RequestBody JSONObject param){
        ...
        if (accessTokenService.isValidTokenAndValidAuthority(authorization, AuthorityUtil.SUPER_ADMIN)!=null) {
        ...
        try {
            productSku = JSON.toJavaObject(param, ProductSku.class);
            if (productSku == null){
        ...
        return productSkuService.addProductSku(userId,productSku);
    }
    ...
    public @ResponseBody JSONObject deleteProductSku(@PathVariable String id,@RequestBody JSONObject param){
        String authorization = request.getHeader("Authorization");
        ...
        return productSkuService.deleteProductSkuById(userId,id);
    }

    /**
     * 根据新ProductSku更新id已存在的ProductSku
     * @param id,newProductSku
     */
    ...
    public JSONObject updateProductSku(@PathVariable("id") String id,@RequestBody JSONObject param){
        ...
        try {
            newProductSku = JSON.toJavaObject(param, ProductSku.class);
        ...
        return productSkuService.updateProductSkuById(userId,id,newProductSku);
    }
    ...
    public @ResponseBody JSONObject findProductSkuByPager(@RequestBody JSONObject param){
        ...
        return productSkuService.findProductSkuByList(keyword,orderBy,timeRangeDate.get(0),timeRangeDate.get(1),page,rows);
    }
    ...
    public void export(String keyword,String orderBy,String timeRange,String token,HttpServletResponse response){
        ...
        productSkuService.exportExcel(keyword,orderBy,timeRangeDate.get(0),timeRangeDate.get(1),response);
    }
    ...
    public @ResponseBody JSONObject productSortUpOrDown(@PathVariable String id,@RequestBody JSONObject param) {
        ...
        return productSkuService.productSkuSortUpOrDown(id, upOrDown);
    }
     ...
     */
    @PostMapping
    public @ResponseBody JSONObject confirmProductSku(@RequestBody JSONObject param) {
        String authorization = request.getHeader("Authorization");
        if (accessTokenService.isValidTokenAndRole(authorization,AuthorityUtil.SUPER_ADMIN)==null) {
            return Result.fail(108, "权限认证失败", "您没有权限或token过期");
        }
        ...
        return productSkuService.confirmProductSku(id, confirm);
    }
}